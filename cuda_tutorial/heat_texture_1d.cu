...
// compile options nvcc -O3 heat_texture_1d.cu 
...
texture<float,1,cudaReadModeElementType> texOut;
...
// bind device allocations to texture references
    cudaBindTexture( NULL, texIn,  dev_tempIn,  DIM*DIM * sizeof(float) );
    cudaBindTexture( NULL, texOut, dev_tempOut, DIM*DIM * sizeof(float) );
