...
// gcc -std=c99 -I/usr/include -L/usr/lib64 -lm -lfftw3 fftw_double_r2c_sine.c
...
int main(void)
{
    ...
    for (int i = 0; i < N; ++i) in[i] = sin(TWOPI*(double) i/(double) N);

    in_file = fopen("fftw_double_r2c_sine_in.dat","w+");
    ...
    out_file = fopen("fftw_double_r2c_sine_out.dat","w+");
